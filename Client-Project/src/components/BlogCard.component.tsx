    ...
    >
      <div className=" w-full">
        <div className="flex gap-2 items-center mb-7 ">
          ...
          <p className="min-w-fit">{Getthedate(latest.createdAt)}</p>
        </div>
        <h1 className="blog-title ">{latest.title}</h1>
        <p className="my-3 text-2xl font-gelasio leading-7 max-sm:hidden md:max-[1101px]:hidden line-clamp-2 ">
          {latest.description}
        </p>
        <div className="flex gap-4 mt-7">
          {latest.tags[0] === " " ? null : (
            <span className=" btn-light py-1 px-4">{latest.tags[0]}</span>
          )}
          <span className=" flex ml-3 items-center gap-2 text-dark-grey ">
            {latest.activity.total_likes}
            <svg
